...
export async function fillDummyPatients() {
...
export async function fillDummyForm() {
    const db = await SQLite.openDatabaseAsync(DB_NAME);
    await db.execAsync(`
...
export async function fillDummyCohort() {
    const db = await SQLite.openDatabaseAsync(DB_NAME);
    await db.execAsync(`
...
export async function fillDummyVariables() {
    const db = await SQLite.openDatabaseAsync(DB_NAME);
    await db.execAsync(baseVariables);
}

export async function resetDB_user_version() {
    const db = await SQLite.openDatabaseAsync(DB_NAME);
